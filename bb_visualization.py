"""Plotly-Visualisierungen: Suchbaum (Kernvisual, Schritt-für-Schritt animierbar) und
Konvergenz des bisher besten Fundes gegen das bewiesene Optimum."""
...
def _compute_layout(nodes):
    """Klassisches Baum-Layout: y = Tiefe, x = Mittelwert der Kind-Positionen
    (Blätter bekommen fortlaufende x-Slots in Besuchsreihenfolge - die
    Knoten-IDs sind bereits in Tiefensuche-Reihenfolge vergeben, 'include'
    vor 'exclude', also von links nach rechts genau die Suchreihenfolge)."""
...
def build_tree_figure(instance, result, step, render_cap):
    ...
    # Layout is computed once over the full rendered node set (independent of
    # the current animation step) so that x/y positions never shift as more
    # nodes are revealed - only which nodes/edges are drawn changes per step.
